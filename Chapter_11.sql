...
-- ## TRY IT YOURSELF
-- Q1.
SELECT date_part('hour', trip_length) as trip_length_hour,
        AVG(trip_length),
        COUNT(*)
FROM (
    SELECT trip_id, 
            tpep_pickup_datetime AS trip_hour,
            date_part('epoch', tpep_dropoff_datetime - tpep_pickup_datetime) * interval '1 second' 
                        AS trip_length
    FROM nyc_yellow_taxi_trips_2016_06_01
    ORDER BY trip_length DESC
    ) AS t
GROUP BY trip_length_hour
ORDER BY trip_length_hour DESC;

/*
There are many long hour trips - 431 of 23+ hr trips! What is this case?
*/

-- Q2.
SELECT future_time AS new_york_time,
        future_time AT TIME ZONE 'Europe/London' AS london_time,
        future_time AT TIME ZONE 'Africa/Johannesburg' AS johannesburg_time,
        future_time AT TIME ZONE 'Europe/Moscow' AS moscow_time,
        future_time AT TIME ZONE 'Australia/Melbourne' AS Melbourne_time
FROM
    (SELECT 'January 1, 2100'::timestamptz AS future_time) as t

-- Q3.
SELECT round(
             corr(trip_length, total_amount)::numeric, 2
            ) AS total_amount_vs_trip_length,
        round(
             corr(trip_distance, total_amount)::numeric, 2
            ) AS total_amount_vs_trip_dist
FROM (
    SELECT trip_id, 
        tpep_pickup_datetime AS trip_hour,
